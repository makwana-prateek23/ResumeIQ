...
function ResumeWorkspace({ mode = 'create', initialResumeData = null }) {
  ...
  const pageFit = useMemo(() => {
    const pageHeight = style.pageSize === 'a4' ? 842 : 792;
    const usableHeight = pageHeight - (style.margin * 2);
    const charsPerLine = Math.max(55, Math.floor(92 * ((612 - style.margin * 2) / 540) * (10 / style.size)));
    const wrappedLines = (value, indent = 0) => value ? String(value).split(/\r?\n/).reduce((total, line) => total + Math.max(1, Math.ceil(line.length / Math.max(40, charsPerLine - indent))), 0) : 0;
    const summaryLines = wrappedLines(resume.summary);
    const skillLines = wrappedLines(resume.skills);
    const experienceLines = resume.experience.reduce((total, item) => total + 1 + item.bullets.filter(Boolean).reduce((count, bullet) => count + wrappedLines(bullet, 8), 0), 0);
    const educationLines = resume.education.filter((item) => item.degree || item.school || item.year).length;
    const customLines = (resume.customSections || []).reduce((total, section) => total + wrappedLines(section.content), 0);
    const visibleSections = [resume.summary, resume.experience.some((item) => item.bullets.some(Boolean) || item.role || item.company), resume.skills, educationLines, customLines].filter(Boolean).length;
    const estimatedHeight = 72 + ((summaryLines + skillLines + experienceLines + educationLines + customLines) * style.size * style.spacing) + (visibleSections * (16 + style.sectionGap)) + (Math.max(0, resume.experience.length - 1) * style.itemGap);
    const pages = Math.max(1, Math.ceil(estimatedHeight / usableHeight));
    const fill = Math.min(100, Math.round((estimatedHeight / (usableHeight * pages)) * 100));
    const message = pages === 1 ? (fill < 65 ? 'The page has room. You can use Balanced for easier reading.' : fill > 95 ? 'The page is very full. Check the final lines before downloading.' : 'The résumé should fit comfortably on one page.') : (fill < 60 ? `The last of ${pages} pages may have too much empty space.` : `The résumé is expected to use ${pages} pages.`);
    return { pages, fill, message };
  }, [resume, style]);
  ...
  return <main className="mx-auto max-w-[1500px] px-4 py-7 sm:px-7">
    ...
    <section className="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-5" aria-labelledby="ats-readiness-title"><div className="flex flex-wrap items-center justify-between gap-3"><div><p id="ats-readiness-title" className="text-sm font-black text-emerald-950">ATS readiness: {atsReadiness.score}%</p><p className="mt-1 text-xs text-emerald-800">This checks résumé basics. Match the wording to each real job description for the strongest result.</p></div><button type="button" onClick={() => applyLayoutPreset('compact')} className="rounded-xl border border-emerald-300 bg-white px-4 py-2 text-xs font-black text-emerald-800 hover:bg-emerald-100">Fit more on one page</button></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{atsReadiness.checks.map((check) => <div key={check.label} className={`flex gap-2 rounded-xl p-3 text-xs font-semibold ${check.passed ? 'bg-white/70 text-emerald-800' : 'border border-amber-200 bg-amber-50 text-amber-900'}`}><span aria-hidden="true" className="font-black">{check.passed ? '✓' : '!'}</span><span>{check.label}</span></div>)}</div></section>
    <div className="grid items-start gap-6 xl:grid-cols-[minmax(400px,0.9fr)_minmax(560px,1.1fr)]">
      ...
      <aside className="self-start">
        <section className={`mb-3 rounded-2xl border p-4 shadow-sm ${pageFit.pages === 1 && pageFit.fill <= 95 ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`} aria-live="polite"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wider text-slate-500">Page check</p><p className="mt-1 text-lg font-black text-slate-900">About {pageFit.pages} {pageFit.pages === 1 ? 'page' : 'pages'} · {pageFit.fill}% full</p><p className="mt-1 text-xs leading-5 text-slate-600">{pageFit.message}</p></div><span className="grid h-12 w-10 shrink-0 place-items-center rounded border border-slate-300 bg-white text-xs font-black text-slate-600 shadow-sm">{pageFit.pages}</span></div>{pageFit.pages > 1 && <button type="button" onClick={() => applyLayoutPreset('compact')} className="mt-3 w-full rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-black text-white hover:bg-indigo-700">Try to fit on one page</button>}</section>
        ...
        <div className="mb-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-bold text-slate-700">How much should fit on the page?</p><p className="mt-0.5 text-xs text-slate-500">Choose one option. The résumé updates immediately.</p><div className="mt-3 grid grid-cols-3 gap-2">{Object.entries({ compact: ['Fit one page', 'Small and tight'], professional: ['Balanced', 'Best default'], spacious: ['More space', 'Larger and open'] }).map(([key, [label, note]]) => <button key={key} type="button" onClick={() => applyLayoutPreset(key)} className={`rounded-xl border p-2 text-left ${style.preset === key ? 'border-indigo-500 bg-indigo-50 ring-2 ring-indigo-100' : 'border-slate-200 hover:border-indigo-300'}`}><span className="block text-xs font-extrabold text-slate-800">{label}</span><span className="block text-[10px] text-slate-500">{note}</span></button>)}</div><details className="mt-4 border-t border-slate-100 pt-3"><summary className="cursor-pointer text-xs font-bold text-indigo-700">More page options</summary><p className="mt-2 text-[11px] leading-4 text-slate-500">You can leave these unchanged. Use them only if the résumé still has too much empty space or moves onto another page.</p><div className="mt-3 grid gap-3 sm:grid-cols-3"><label className="text-xs font-bold text-slate-600">Paper size<select value={style.pageSize} onChange={(e) => updateStyle('pageSize', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 p-2"><option value="letter">US Letter</option><option value="a4">A4</option></select></label><label className="text-xs font-bold text-slate-600">Space around the page<select value={style.margin} onChange={(e) => updateStyle('margin', Number(e.target.value))} className="mt-1 w-full rounded-lg border border-slate-200 p-2"><option value="30">Less</option><option value="36">Normal</option><option value="48">More</option><option value="60">Most</option></select></label><label className="text-xs font-bold text-slate-600">Space between sections<select value={style.sectionGap} onChange={(e) => updateStyle('sectionGap', Number(e.target.value))} className="mt-1 w-full rounded-lg border border-slate-200 p-2"><option value="10">Less</option><option value="14">Normal</option><option value="20">More</option></select></label><label className="text-xs font-bold text-slate-600">Space between jobs<select value={style.itemGap} onChange={(e) => updateStyle('itemGap', Number(e.target.value))} className="mt-1 w-full rounded-lg border border-slate-200 p-2"><option value="4">Less</option><option value="6">Normal</option><option value="10">More</option></select></label><label className="text-xs font-bold text-slate-600">Bullet position<select value={style.bulletIndent} onChange={(e) => updateStyle('bulletIndent', Number(e.target.value))} className="mt-1 w-full rounded-lg border border-slate-200 p-2"><option value="6">Near the edge</option><option value="10">Normal</option><option value="16">Further in</option></select></label></div></details></div>
