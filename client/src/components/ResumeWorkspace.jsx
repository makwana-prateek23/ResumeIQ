...
const initialStyle = { font: 'Arial', size: 10.5, spacing: 1.45, accent: '#3730a3', margin: 34, template: 'classic' };
...
function ResumeWorkspace({ mode = 'create' }) {
  ...
  const [style, setStyle] = useState(() => {
    try { return JSON.parse(localStorage.getItem(storageKey))?.style || initialStyle; } catch { return initialStyle; }
  });
  ...
  return <main className="mx-auto max-w-[1500px] px-4 py-7 sm:px-7">
    <header className="mb-6 flex flex-col gap-4 rounded-3xl bg-slate-950 p-6 text-white sm:p-8 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-300">{mode === 'format' ? 'Resume formatting studio' : 'Guided resume builder'}</p><h1 className="mt-2 text-3xl font-black tracking-tight">{mode === 'format' ? 'Make every page clean and consistent.' : 'Build your resume, one simple block at a time.'}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Fill in the blocks on the left. Your professional, ATS-friendly document updates instantly on the right.</p></div><div className="min-w-40"><div className="flex justify-between text-xs font-bold"><span>Resume complete</span><span>{completion}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-cyan-400 transition-all" style={{ width: `${completion}%` }} /></div></div></header>
    <div className="grid items-start gap-6 xl:grid-cols-[minmax(400px,0.9fr)_minmax(560px,1.1fr)]">
      ...
      <aside className="xl:sticky xl:top-24">
        ...
        <div className="max-h-[calc(100vh-210px)] overflow-auto rounded-2xl bg-slate-200 p-3 sm:p-6"><article className="mx-auto min-h-[842px] max-w-[595px] bg-white shadow-2xl" style={{ padding: `${style.margin}px`, fontFamily: style.font, fontSize: `${style.size}px`, lineHeight: style.spacing }}><header className={style.template === 'modern' ? 'text-left' : 'text-center'}><h1 className="text-[2.25em] font-black tracking-tight text-slate-950">{resume.name || 'YOUR NAME'}</h1><p className="mt-1 text-[1.2em] font-bold" style={{ color: style.accent }}>{resume.role || 'TARGET ROLE'}</p><p className="mt-2 text-[0.9em] text-slate-500">{contact || 'City, State · phone · email · LinkedIn'}</p></header><ResumeSection title="Professional Summary" color={style.accent}><p>{resume.summary || 'Write a focused 2–4 line summary that highlights your experience, specialization, and strongest result.'}</p></ResumeSection><ResumeSection title="Experience" color={style.accent}>{resume.experience.map((item) => <div key={item.id} className="mb-4"><div className="flex justify-between gap-4 font-bold text-slate-950"><span>{item.role || 'ROLE TITLE'}{item.company ? ` · ${item.company}` : ' · COMPANY'}</span><span className="whitespace-nowrap">{item.start || 'START'} – {item.end || 'END'}</span></div><ul className="mt-1 list-disc pl-5">{item.bullets.map((bullet, index) => <li key={index}>{bullet || 'Describe what you achieved, how you did it, and the measurable result.'}</li>)}</ul></div>)}</ResumeSection><ResumeSection title="Skills" color={style.accent}><p>{resume.skills || 'Add relevant skills separated by commas.'}</p></ResumeSection><ResumeSection title="Education" color={style.accent}>{resume.education.map((item) => <p key={item.id} className="font-bold">{item.degree || 'DEGREE'} · {item.school || 'INSTITUTION'} <span className="float-right">{item.year || 'YEAR'}</span></p>)}</ResumeSection></article></div>
      </aside>
    </div>
  </main>;
}
...
function ResumeSection({ title, color, children }) {
  return <section className="mt-6 text-slate-700"><h2 className="mb-2 border-b pb-1 text-[1.05em] font-black uppercase tracking-[0.14em]" style={{ color, borderColor: `${color}55` }}>{title}</h2>{children}</section>;
}
