...
function ResumeWorkspace({ mode = 'create', initialResumeData = null }) {
  ...
  const [celebration, setCelebration] = useState({ confetti: [], reduceMotion: false });

  useEffect(() => {
    if (prevCompletionRef.current < 100 && completion === 100 && !hasCelebratedRef.current) {
      const reduceMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false;
      setCelebration({ confetti: reduceMotion ? [] : generateConfetti(accent === 'mono'), reduceMotion });
        ...
        lines.forEach((line, index) => {
          ...
          if (index === lines.length - 1 || words.length < 2) {
            ...
            words.forEach((word, wordIndex) => {
              pdf.text(word, x, y);
              x += pdf.getTextWidth(word) + (wordIndex < words.length - 1 ? balancedGap : 0);
            });
          }
          y += layout.lineHeight;
        });
      ...
      const wordSections = {
        ...
        experience: () => {
          ...
          items.forEach((item, itemIndex) => {
            ...
            children.push(paragraph(`${title}${dates ? `\t${dates}` : ''}`, { bold: true, tabs: true, after: bullets.length && !item.location ? 60 : hasFollowingItem && !item.location ? Math.round(layout.itemGap * 20) : 0, keepNext: bullets.length > 0 || Boolean(item.location) }));
            if (item.location) children.push(paragraph(item.location, { size: 18, after: bullets.length ? 60 : (hasFollowingItem ? Math.round(layout.itemGap * 20) : 0), keepNext: bullets.length > 0 }));
        ...
        skills: () => {
          ...
          rows.forEach((row, index) => children.push(new Paragraph({
            spacing: { after: index < rows.length - 1 ? 20 : 0, line: Math.round(layout.bodySize * 20 * layout.spacing), lineRule: LineRuleType.EXACT },
            children: [
      ...
      <aside className="self-start">
        ...
        <div className="mb-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-3">
            ...
            <div className="text-xs font-bold text-slate-600"><span>Text size</span><div className="mt-1 flex overflow-hidden rounded-lg border border-slate-200 bg-white"><button type="button" aria-label="Decrease resume font size" disabled={layout.bodySize <= 9} onClick={() => updateAppearance('size', Math.max(9, layout.bodySize - 0.5))} className="grid w-9 place-items-center border-r border-slate-200 text-base font-black hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-35">−</button><select aria-label="Resume text size" value={layout.bodySize} onChange={(e) => updateAppearance('size', Number(e.target.value))} className="min-w-0 flex-1 bg-white p-2 text-xs font-bold outline-none"><option value="9">9 pt</option><option value="9.5">9.5 pt</option><option value="10">10 pt</option><option value="10.5">10.5 pt</option><option value="11">11 pt</option><option value="11.5">11.5 pt</option><option value="12">12 pt</option></select><button type="button" aria-label="Increase resume font size" disabled={layout.bodySize >= 12} onClick={() => updateAppearance('size', Math.min(12, layout.bodySize + 0.5))} className="grid w-9 place-items-center border-l border-slate-200 text-base font-black hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-35">+</button></div></div>
