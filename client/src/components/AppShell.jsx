...
function AppShell() {
  ...
  function requireResume(event) {
    ...
    navigate('/');
    window.setTimeout(() => document.querySelector('#resume-review')?.scrollIntoView({ behavior: 'smooth' }), 50);
  }
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-3 sm:px-8 lg:flex-row lg:items-center lg:justify-between">
        ...
        <nav aria-label="Main navigation" className="flex overflow-x-auto rounded-xl bg-slate-100 p-1">
          {links.map(([to, label]) => <NavLink key={to} to={to} end={to === '/'} onClick={to === '/format' ? requireResume : undefined} className={({ isActive }) => `whitespace-nowrap rounded-lg px-4 py-2 text-sm font-bold transition ${isActive ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>{label}</NavLink>)}
        </nav>
