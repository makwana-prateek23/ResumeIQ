  ...
  return <div className="min-h-screen bg-[#f5f7fb] text-slate-900">
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-3 py-3 sm:gap-4 sm:px-8">
        <NavLink to="/" className="flex min-w-0 items-center gap-2 no-underline sm:gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-(--accent-1) to-(--accent-2) font-black text-white shadow-lg shadow-indigo-200 sm:h-10 sm:w-10">R</span><div className="min-w-0"><p className="truncate text-sm font-extrabold tracking-tight sm:text-base">ResumeIQ</p><p className="hidden text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 sm:block">Resume workspace</p></div></NavLink>
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-3"><ThemePicker />{isAuthenticated ? <div className="flex items-center gap-1.5 sm:gap-2"><span className="hidden h-9 w-9 place-items-center overflow-hidden rounded-full bg-indigo-100 text-xs font-black text-indigo-700 min-[420px]:grid">{user.avatarUrl ? <img src={user.avatarUrl} alt="" className="h-full w-full object-cover" /> : user.name?.slice(0, 1).toUpperCase()}</span><span className="hidden max-w-28 truncate text-xs font-bold text-slate-700 xl:block">{user.name}</span><button onClick={() => { logout(); navigate('/'); }} className="min-h-9 rounded-lg border border-slate-200 px-2.5 py-2 text-xs font-bold text-slate-600 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700 sm:px-3">Sign out</button></div> : <div className="flex items-center gap-1 sm:gap-2"><NavLink to="/login" className="rounded-lg px-2 py-2 text-xs font-black text-slate-600 hover:bg-slate-100 sm:px-3">Sign in</NavLink><NavLink to="/signup" className="rounded-lg bg-slate-950 px-2.5 py-2 text-xs font-black text-white transition hover:-translate-y-0.5 sm:px-3">Sign up</NavLink></div>}</div>
      </div>
    </header>
