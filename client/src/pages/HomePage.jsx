...
function HomePage() {
  ...
  function selectResume(event) {
    ...
    if (file && (file.type !== 'application/pdf' || !file.name.toLowerCase().endsWith('.pdf'))) { event.target.value = ''; setResume(null); return setError('Choose a valid PDF resume.'); }
    if (file && file.size > 5 * 1024 * 1024) { event.target.value = ''; setResume(null); return setError('Resume PDF must be 5 MB or smaller.'); }
    setResume(file);
  }

  async function submitAnalysis(event) {
    event.preventDefault(); setError(''); setResult(null);
    if (!resume) return setError('Choose a resume PDF.');
  ...
  function downloadReport() {
    ...
    link.href = url;
    link.download = `resumeiq-report-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
    ...
    <div className="relative z-10 mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-12">
      ...
      <div className="mt-8 grid items-start gap-8 lg:grid-cols-[420px_minmax(0,1fr)]">
        <form onSubmit={submitAnalysis} className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-[0_24px_70px_-35px_rgba(15,23,42,0.4)] lg:sticky lg:top-6 sm:p-7">
          ...
          <label htmlFor="resume" className={`mt-5 flex cursor-pointer flex-col items-center rounded-2xl border-2 border-dashed p-6 text-center transition ${resume ? 'border-emerald-300 bg-emerald-50' : 'border-slate-300 bg-slate-50 hover:border-indigo-400 hover:bg-indigo-50/50'}`}><span className="grid h-12 w-12 place-items-center rounded-2xl bg-white text-xl shadow-sm">PDF</span><span className="mt-3 text-sm font-bold">{resume ? resume.name : 'Choose your resume PDF'}</span><span className="mt-1 text-xs text-slate-500">Maximum 5 MB, processed in memory</span></label><input id="resume" name="resume" type="file" accept="application/pdf,.pdf" onChange={selectResume} className="sr-only" />
        ...
        <div aria-live="polite" className="print-report grid gap-6">
          <div className="no-print flex flex-wrap justify-end gap-3"><button type="button" onClick={downloadReport} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm hover:border-indigo-300 hover:text-indigo-700">Download JSON</button><button type="button" onClick={downloadPdfReport} disabled={isDownloadingPdf} className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-bold text-white shadow-sm hover:bg-indigo-700 disabled:cursor-wait disabled:opacity-60">{isDownloadingPdf ? 'Creating PDF...' : 'Download PDF'}</button></div>
          ...
          <Panel title="Exact resume changes" className="border-indigo-200 bg-gradient-to-br from-white to-indigo-50/60"><div className="grid gap-3">{result.tailoringPlan.keywordActions.slice(0, 8).map((item, index) => <article key={item.term} className="rounded-2xl border border-indigo-100 bg-white p-5 shadow-sm"><div className="flex gap-3"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-indigo-600 text-xs font-bold text-white">{index + 1}</span><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold">{item.term}</h3><span className="text-[10px] font-bold uppercase tracking-wider text-indigo-500">{item.priority} / {item.recommendedSection}</span></div><p className="mt-2 text-sm leading-6 text-slate-600">{item.guidance}</p>{item.bulletPrompt && <p className="mt-3 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-600">Suggested change: {item.bulletPrompt}</p>}</div></div></article>)}</div></Panel>
